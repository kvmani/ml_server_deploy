...
# Versions capped to what the office pip mirror actually carries.
CONSTRAINTS_FILE = Path(__file__).resolve().parents[1] / "requirements" / "constraints.txt"
...
def read_constraints(path: Path) -> dict[str, str]:
    """Return the exact `name==version` pins in a pip constraints file.

    Names are normalised the way pip compares them, so `ThreadPoolCtl` and
    `threadpoolctl` are the same package.
    """
    pins: dict[str, str] = {}
    if not path.is_file():
        return pins
...
def constraint_violations(frozen: str, pins: dict[str, str]) -> list[str]:
    """Every package in `pip freeze` output pinned differently from a constraint.

    `-c` already makes pip honour these during the install. This check is what
    stops a later change to the install step -- a dropped `-c`, a second install
    that upgrades in place -- from silently shipping a version the office mirror
    cannot serve, which is how suite v1.12.0 failed to deploy.
    """
    problems: list[str] = []
    for line in frozen.splitlines():
        ...
        wanted = pins.get(normalise(name))
        if wanted is not None and version != wanted:
            problems.append(f"{name}=={version} resolved, but constraints.txt requires {name}=={wanted}")
    return problems
...
def main() -> int:
    ...
    try:
        ...
        # Caps imposed by the office mirror. The file sits in requirements/, so
        # build_suite.py ships it inside the release archive as well.
        constraints = CONSTRAINTS_FILE
        pins = read_constraints(constraints)
        for name, version in sorted(pins.items()):
            print(f"    mirror constraint: {name}=={version}")
        ...
        if not args.skip_install:
            ...
            constraint_args = ["-c", str(constraints)] if constraints.is_file() else []
        ...
        violations = constraint_violations(frozen, pins)
        if violations:
            annotate("Office mirror constraint violated", "\n".join(violations))
            raise GateError(
                "the resolved environment does not match requirements/constraints.txt; the office "
                "mirror cannot serve it:\n    " + "\n    ".join(violations)
            )
        ...
        audit.write_text(
            ...
            + "".join(f"# Capped to the mirror by requirements/constraints.txt: {name}=={version}\n"
                      for name, version in sorted(pins.items()))
            + "\n".join(names) + "\n",
            encoding="utf-8", newline="\n",
        )
